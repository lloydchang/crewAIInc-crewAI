...
class CrewAIManager:
    ...
    def _initialize_tool_config(self) -> ToolConfig:
        try:
            ...
            tool_config = ToolConfig(**tool_config_data)  # Ensure this line has correct syntax
            logger.debug(f"[Line 32] ToolConfig successfully initialized: {tool_config}")
            return tool_config
        except ValidationError as ve:
            logger.error(f"[Line 33] Validation error in tool configuration: {ve.errors()}", exc_info=True)
            raise
